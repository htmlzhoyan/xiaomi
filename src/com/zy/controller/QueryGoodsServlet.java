...
public class QueryGoodsServlet extends HttpServlet implements Servlet {
	private static final long serialVersionUID = 1L;
    private IGoodsService gsd = new GoodsService(); 
    /**
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "findId":
			try {
				...
				Goods goods = gsd.findGoodsById(Integer.parseInt(id));
				
				JsonUtils.writeDateJosnInfo(1, goods, response);
		...
		case "findName":
			try {
				...
				Goods goods = gsd.findGoodsByName(name);
				
				JsonUtils.writeJsonUtils(1, goods, response);
			} catch (NumberFormatException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				JsonUtils.writeJsonUtils(0, null, response);
		...
		case "delete":
			try {
				String id = request.getParameter("id");
				gsd.deleteServe(Integer.parseInt(id));
		...
		case "update":	
			try {
				...
				this.addInput(request,response,rest,Integer.valueOf(id));
		...
		case "search":
			...
			String createDates = request.getParameter("times");
			Date date = null;
			Date dates = null;
			if(createDate.equals("")&&createDates.equals("")) {
				date = null;
				dates = null;
			}else {
			...
			List<GoodAndGrade> list =null;
			list= gsd.searchService(name, date,dates);
			JsonUtils.writeJsonUtils(1, list, response);
			break;
		default:
			break;
...
private void addInput(HttpServletRequest request, HttpServletResponse response,Goods rest,int idss) {
